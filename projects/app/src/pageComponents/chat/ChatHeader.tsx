...
const ChatHeader = ({
  ...
  return isPc && isPlugin ? null : (
    <Flex
      ...
      color={'myGray.900'}
      fontSize={'sm'}
      background={'#e6f0ff'} // Changed to light blue to match image
              ...
              apps.map((item) => (
                <Flex justify={'center'} key={item._id}>
                  <Flex
                    ...
                    {...(item._id === appId
                      ? {
                          backgroundColor: '#e6f0ff !important', // Changed to light blue
                          color: '#3182ce' // Changed to blue for selected item
                        }
                      : {
        ...
        <Flex alignItems={'center'} onClick={toggleDrawer}>
          ...
          {isShareChat ? null : (
            <MyIcon
              ...
              w={'1.25rem'}
              color={isOpenDrawer ? '#3182ce' : '#4A5568'} // Changed colors
            />
          )}
        </Flex>
      </Flex>
    ...
    <>
      ...
      {!!chatModels && chatModels.length > 0 && (
        <MyTooltip label={chatModels.join(',')}>
          <MyTag ml={2} colorSchema={'blue'} bg={'rgba(49, 130, 206, 0.1)'} color={'#3182ce'}>
            <MyIcon name={'core/chat/chatModelTag'} w={'14px'} color={'#3182ce'} />
